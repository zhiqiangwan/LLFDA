...
def ssd_512(image_size,
    ...
    if aspect_ratios_global is None and aspect_ratios_per_layer is None:
        raise ValueError("`aspect_ratios_global` and `aspect_ratios_per_layer` \
    ...
    def calculate_GAP_SUB(tensor, batch_size, name):
        source = Lambda(lambda tensor: tensor[:batch_size, :, :, :])(tensor)
        target = Lambda(lambda tensor: tensor[batch_size:, :, :, :])(tensor)
        source_global_pool = GlobalAveragePooling2D()(source)
        target_global_pool = GlobalAveragePooling2D()(target)
        source_substract_target = Subtract(name=name)([source_global_pool, target_global_pool])
        return source_substract_target
    ...
    conv1_1_source_substract_target = calculate_GAP_SUB(conv1_1, batch_size, 'contrastive_loss1')
    predicted_output.append(conv1_1_source_substract_target)
    ...
    conv2_1_source_substract_target = calculate_GAP_SUB(conv2_1, batch_size, 'contrastive_loss2')
    predicted_output.append(conv2_1_source_substract_target)
    ...
    conv8_1 = Conv2D(128, (1, 1), activation='relu', padding='same', kernel_initializer='he_normal',
                     kernel_regularizer=l2(l2_reg), name='conv8_1')(conv7_2)  # conv7_2
    ...
    conv9_1 = Conv2D(128, (1, 1), activation='relu', padding='same', kernel_initializer='he_normal',
                     kernel_regularizer=l2(l2_reg), name='conv9_1')(conv8_2)  # conv8_2
    ...
    conv4_3_norm_source = Lambda(lambda tensor: tensor[:batch_size, :, :, :])(conv4_3_norm)
    conv4_3_norm_mbox_conf = Conv2D(n_boxes[0] * n_classes, (3, 3), padding='same', kernel_initializer='he_normal',
                                    kernel_regularizer=l2(l2_reg), name='conv4_3_norm_mbox_conf')(conv4_3_norm_source)  # conv4_3_norm

    fc7_source = Lambda(lambda tensor: tensor[:batch_size, :, :, :])(fc7)
    fc7_mbox_conf = Conv2D(n_boxes[1] * n_classes, (3, 3), padding='same', kernel_initializer='he_normal',
                           kernel_regularizer=l2(l2_reg), name='fc7_mbox_conf')(fc7_source)  # fc7

    conv6_2_source = Lambda(lambda tensor: tensor[:batch_size, :, :, :])(conv6_2)
    conv6_2_mbox_conf = Conv2D(n_boxes[2] * n_classes, (3, 3), padding='same', kernel_initializer='he_normal',
                               kernel_regularizer=l2(l2_reg), name='conv6_2_mbox_conf')(conv6_2_source)  # conv6_2

    conv7_2_source = Lambda(lambda tensor: tensor[:batch_size, :, :, :])(conv7_2)
    conv7_2_mbox_conf = Conv2D(n_boxes[3] * n_classes, (3, 3), padding='same', kernel_initializer='he_normal',
                               kernel_regularizer=l2(l2_reg), name='conv7_2_mbox_conf')(conv7_2_source)  # conv7_2

    conv8_2_source = Lambda(lambda tensor: tensor[:batch_size, :, :, :])(conv8_2)
    conv8_2_mbox_conf = Conv2D(n_boxes[4] * n_classes, (3, 3), padding='same', kernel_initializer='he_normal',
                               kernel_regularizer=l2(l2_reg), name='conv8_2_mbox_conf')(conv8_2_source)  # conv8_2

    conv9_2_source = Lambda(lambda tensor: tensor[:batch_size, :, :, :])(conv9_2)
    conv9_2_mbox_conf = Conv2D(n_boxes[5] * n_classes, (3, 3), padding='same', kernel_initializer='he_normal',
                               kernel_regularizer=l2(l2_reg), name='conv9_2_mbox_conf')(conv9_2_source)  # conv9_2

    conv10_2_source = Lambda(lambda tensor: tensor[:batch_size, :, :, :])(conv10_2)
    conv10_2_mbox_conf = Conv2D(n_boxes[6] * n_classes, (3, 3), padding='same', kernel_initializer='he_normal',
                                kernel_regularizer=l2(l2_reg), name='conv10_2_mbox_conf')(conv10_2_source)  # conv10_2
    # We predict 4 box coordinates for each box, hence the localization predictors have depth `n_boxes * 4`
    # Output shape of the localization layers: `(batch, height, width, n_boxes * 4)`
    conv4_3_norm_mbox_loc = Conv2D(n_boxes[0] * 4, (3, 3), padding='same', kernel_initializer='he_normal',
                                   kernel_regularizer=l2(l2_reg), name='conv4_3_norm_mbox_loc')(conv4_3_norm_source)
    fc7_mbox_loc = Conv2D(n_boxes[1] * 4, (3, 3), padding='same', kernel_initializer='he_normal',
                          kernel_regularizer=l2(l2_reg), name='fc7_mbox_loc')(fc7_source)
    conv6_2_mbox_loc = Conv2D(n_boxes[2] * 4, (3, 3), padding='same', kernel_initializer='he_normal',
                              kernel_regularizer=l2(l2_reg), name='conv6_2_mbox_loc')(conv6_2_source)
    conv7_2_mbox_loc = Conv2D(n_boxes[3] * 4, (3, 3), padding='same', kernel_initializer='he_normal',
                              kernel_regularizer=l2(l2_reg), name='conv7_2_mbox_loc')(conv7_2_source)
    conv8_2_mbox_loc = Conv2D(n_boxes[4] * 4, (3, 3), padding='same', kernel_initializer='he_normal',
                              kernel_regularizer=l2(l2_reg), name='conv8_2_mbox_loc')(conv8_2_source)
    conv9_2_mbox_loc = Conv2D(n_boxes[5] * 4, (3, 3), padding='same', kernel_initializer='he_normal',
                              kernel_regularizer=l2(l2_reg), name='conv9_2_mbox_loc')(conv9_2_source)
    conv10_2_mbox_loc = Conv2D(n_boxes[6] * 4, (3, 3), padding='same', kernel_initializer='he_normal',
                               kernel_regularizer=l2(l2_reg), name='conv10_2_mbox_loc')(conv10_2_source)
