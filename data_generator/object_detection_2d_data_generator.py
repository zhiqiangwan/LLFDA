    ...
    def __init__(self,
        ...
        if not filenames is None:
            if isinstance(filenames, (list, tuple)):
                ...
                with open(filenames, 'r') as f: # open(filenames, 'rb') as f:
            ...
            for image_id in it:
                ...
                if not annotations_dir is None:
                    ...
                    for obj in objects:
                        ...
                        pose = 'Unspecified'#obj.find('pose', recursive=False).text
            ...
            if not (self.labels is None):
                ...
                if resize:
                    if len(labels) != 0:
                        ...
                        labels[:, [ymin, ymax]] = np.round(labels[:, [ymin, ymax]] * (resize[0] / img_height_bf_resize), decimals=0)
                        labels[:, [xmin, xmax]] = np.round(labels[:, [xmin, xmax]] * (resize[1] / img_width_bf_resize), decimals=0)
    ...
    def generate(self,
        ...
        while True:
            ...
            if not (self.images is None):  # Load images from memory. Not supported yet
                ...
                raise ValueError('Not supported yet.')  # Load the individual image from disk.
                batch_filenames = self.filenames[current:current+batch_size]
            ...
            batch_X_foggy = np.array(batch_X_foggy)

            if (batch_X.size == 0 or batch_X_foggy.size == 0):
            ...
            # Compose the output.
            #########################################################################################
            if batch_X.shape[0] != batch_X_foggy.shape[0]:
                raise Exception('batch_X and batch_X_foggy should all have the same number of samples.')
            ret = []
